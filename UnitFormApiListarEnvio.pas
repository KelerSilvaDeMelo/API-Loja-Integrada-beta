...
type
  TFormApiListarEnvio = class(TForm)
    LayoutApiEnvio: TLayout;
    ...
    FDMemTableEnvio: TFDMemTable;
    ActionListApiEnvio: TActionList;
...
var
  FormApiListarEnvio: TFormApiListarEnvio;
...
{---------------------------------[ INTERNAL ]---------------------------------}
procedure TFormApiListarEnvio.ImageEngrenagemClick(Sender: TObject);
begin
  Self.ActionVoltar.Execute;
end;

{---------------------------------[ ACTION ]-----------------------------------}
procedure TFormApiListarEnvio.ActionRequestExecute(Sender: TObject);
begin
  Self.ExecutaSolicitacaoHttpGet;
end;

procedure TFormApiListarEnvio.ActionVoltarExecute(Sender: TObject);
begin
  Self.LayoutApiEnvio.Parent := Self;
end;

{---------------------------------[ PRIVATE ]----------------------------------}
procedure TFormApiListarEnvio.ExecutaSolicitacaoHttpGet;
var
  BaseURL, UriListarCategorias: string;
  chave, valor: String;
begin
  Self.TabControlAPI.ActiveTab  := Self.TabItemEnvio;
  BaseURL                       := Self.EditURL.Text;
  UriListarCategorias           := Self.EditURI.Text;

  Self.RESTResponseDataSetAdapter1.Active := False;
  Self.FDMemTableEnvio.Active             := False;
...
{---------------------------------[ PUBLIC ]-----------------------------------}
procedure TFormApiListarEnvio.Mostra(Leiaute: TLayout);
begin
  Self.TabControlAPI.ActiveTab  := TabItemEnvio;
  Self.LayoutApiEnvio.Parent    := Leiaute;
end;
