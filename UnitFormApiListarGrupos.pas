...
type
  TFormApiListarGrupos = class(TForm)
    LayoutApiGrupo: TLayout;
    ...
    FDMemTableGrupo: TFDMemTable;
    ActionListApiGrupo: TActionList;
...
var
  FormApiListarGrupos: TFormApiListarGrupos;
...
{---------------------------------[ INTERNAL ]---------------------------------}
procedure TFormApiListarGrupos.ImageEngrenagemClick(Sender: TObject);
begin
  Self.ActionVoltar.Execute;
end;

{---------------------------------[ ACTION ]-----------------------------------}
procedure TFormApiListarGrupos.ActionRequestExecute(Sender: TObject);
begin
  Self.ExecutaSolicitacaoHttpGet;
end;

procedure TFormApiListarGrupos.ActionVoltarExecute(Sender: TObject);
begin
  Self.LayoutApiGrupo.Parent := Self;
end;

{---------------------------------[ PRIVATE ]----------------------------------}
procedure TFormApiListarGrupos.ExecutaSolicitacaoHttpGet;
var
  BaseURL, UriListarCategorias: string;
  chave, valor: String;
begin
  Self.TabControlAPI.ActiveTab  := Self.TabItemGrupo;
  BaseURL                       := Self.EditURL.Text;
  UriListarCategorias           := Self.EditURI.Text;

  Self.RESTResponseDataSetAdapter1.Active := False;
  Self.FDMemTableGrupo.Active             := False;
...
{---------------------------------[ PUBLIC ]-----------------------------------}
procedure TFormApiListarGrupos.Mostra(Leiaute: TLayout);
begin
  Self.TabControlAPI.ActiveTab  := TabItemGrupo;
  Self.LayoutApiGrupo.Parent  := Leiaute;
end;
