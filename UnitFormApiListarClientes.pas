...
unit UnitFormApiListarClientes;
...
type
  TFormApiListarClientes = class(TForm)
    LayoutApiCliente: TLayout;
    ...
    ActionListApiCliente: TActionList;
...
var
  FormApiListarClientes: TFormApiListarClientes;
...
{---------------------------------[ INTERNAL ]---------------------------------}
procedure TFormApiListarClientes.ImageEngrenagemClick(Sender: TObject);
begin
  Self.ActionVoltar.Execute;
end;

{---------------------------------[ ACTION ]-----------------------------------}
procedure TFormApiListarClientes.ActionRequestExecute(Sender: TObject);
begin
  Self.ExecutaSolicitacaoHttpGet;
end;

procedure TFormApiListarClientes.ActionVoltarExecute(Sender: TObject);
begin
  Self.LayoutApiCliente.Parent := Self;
end;

{---------------------------------[ PRIVATE ]----------------------------------}
procedure TFormApiListarClientes.ExecutaSolicitacaoHttpGet;
var
  BaseURL, UriListarCategorias: string;
  chave, valor: String;
begin
  Self.TabControlAPI.ActiveTab  := Self.TabItemCliente;
  BaseURL                       := Self.EditURL.Text;
  UriListarCategorias           := Self.EditURI.Text;

  Self.RESTResponseDataSetAdapter1.Active := False;
  Self.FDMemTableCliente.Active           := False;
...
{---------------------------------[ PUBLIC ]-----------------------------------}
procedure TFormApiListarClientes.Mostra(Leiaute: TLayout);
begin
  Self.TabControlAPI.ActiveTab  := TabItemCliente;
  Self.LayoutApiCliente.Parent  := Leiaute;
end;
