...
type
  TFormApiListarMarcas = class(TForm)
    LayoutApiListarMarcas: TLayout;
    ...
    ActionListApiListarMarcas: TActionList;
...
var
  FormApiListarMarcas: TFormApiListarMarcas;
...
{---------------------------------[ INTERNAL ]---------------------------------}
procedure TFormApiListarMarcas.ImageEngrenagemClick(Sender: TObject);
begin
  Self.ActionVoltar.Execute;
end;

{---------------------------------[ ACTION ]-----------------------------------}
procedure TFormApiListarMarcas.ActionRequestExecute(Sender: TObject);
begin
  Self.ExecutaSolicitacaoHttpGet;
end;

procedure TFormApiListarMarcas.ActionVoltarExecute(Sender: TObject);
begin
  Self.LayoutApiListarMarcas.Parent := Self;
end;

{---------------------------------[ PRIVATE ]----------------------------------}
procedure TFormApiListarMarcas.ExecutaSolicitacaoHttpGet;
var
  BaseURL, UriListarCategorias: string;
  chave, valor: String;
begin
  Self.TabControlAPI.ActiveTab  := Self.TabItemListaMarca;
  BaseURL                       := Self.EditURL.Text;
  UriListarCategorias           := Self.EditURI.Text;

  Self.RESTResponseDataSetAdapter1.Active := False;
  Self.FDMemTableMarcas.Active            := False;
...
{---------------------------------[ PUBLIC ]-----------------------------------}
procedure TFormApiListarMarcas.Mostra(Leiaute: TLayout);
begin
  Self.TabControlAPI.ActiveTab      := TabItemListaMarca;
  Self.LayoutApiListarMarcas.Parent := Leiaute;
end;
