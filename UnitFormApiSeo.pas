...
type
  TFormApiSeo = class(TForm)
    LayoutApiSeo: TLayout;
    ...
    FDMemTableSeo: TFDMemTable;
    ActionListApiSeo: TActionList;
...
{---------------------------------[ INTERNAL ]---------------------------------}
procedure TFormApiSeo.ImageEngrenagemClick(Sender: TObject);
begin
  Self.ActionVoltar.Execute;
end;

{---------------------------------[ ACTION ]-----------------------------------}
procedure TFormApiSeo.ActionRequestExecute(Sender: TObject);
begin
  Self.ExecutaSolicitacaoHttpGet;
end;

procedure TFormApiSeo.ActionVoltarExecute(Sender: TObject);
begin
  Self.LayoutApiSeo.Parent := Self;
end;

{---------------------------------[ PRIVATE ]----------------------------------}
procedure TFormApiSeo.ExecutaSolicitacaoHttpGet;
var
  BaseURL, UriListarCategorias: string;
  chave, valor: String;
begin
  Self.TabControlAPI.ActiveTab  := Self.TabItemSeo;
  BaseURL                       := Self.EditURL.Text;
  UriListarCategorias           := Self.EditURI.Text;

  Self.RESTResponseDataSetAdapter1.Active := False;
  Self.FDMemTableSeo.Active               := False;
...
{---------------------------------[ PUBLIC ]-----------------------------------}
procedure TFormApiSeo.Mostra(Leiaute: TLayout);
begin
  Self.TabControlAPI.ActiveTab  := TabItemSeo;
  Self.LayoutApiSeo.Parent      := Leiaute;

  ShowMessage( 'Falta definir o SEO' );
  Self.ActionVoltar.Execute;
end;
